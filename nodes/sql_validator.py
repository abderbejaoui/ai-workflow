...
class SQLValidator:
    ...
    def _validate_tables(self, sql: str, schema_cache: Dict[str, Any]) -> List[str]:
        ...
        # Pattern: FROM/JOIN schema.table_name or table_name
        ...
        for table in available_tables:
            # Add both simple and fully qualified names
            available_table_names.add(table.get("table", "").lower())
            available_table_names.add(table.get("full_name", "").lower())
            
            # Also add schema.table format if present
            schema = table.get("schema", "")
            table_name = table.get("table", "")
            if schema and table_name:
                available_table_names.add(f"{schema}.{table_name}".lower())
